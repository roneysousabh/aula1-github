...
if ($link === false){
	die("ERRO: Não foi possível conectar ao BD.").mysqli_connect_erro());
{
	else
}
}
...
$sql = "INSERT INTO FUNCIONARIO (CODIGO, NOME, CARGO, DESCRICAOCARGO, SETOR, SALARIO) VALUES ('$codigo', '$nome', '$cargo', '$descCargo', '$setor', '$salario')";
if(mysqli_query($link, $sql)){
	echo "Gravação efetuada com sucesso!"; //aqui poderia ser incluído um código para redirect
} else {
	echo "Erro (Não foi possível inserir o registro na tabela) $sql".mysqli_erro($link);
}
//Close connection
mysqli_close($link);
 ?>